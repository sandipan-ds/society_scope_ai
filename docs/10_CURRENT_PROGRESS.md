...
| Vector retrieval | 🟡 | `app/retrieval/vector_store.py` (add/delete/count/reset) — query interface comes with chat routing |
...
| Audit logging | 🟡 | Login/register/me + `query_private` + `upload_document`/`delete_document` events logged; chat/refusal logging pending |
