...
| Eval harness | ✅ | `data/eval_queries/eval_queries.jsonl` (70 cases: 20 public / 20 private / 20 unauthorized / 10 hybrid) + `scripts/run_eval.py` — scores routing, citations, refusals, leakage, content; 70/70 passing |
...
10. ✅ Add evaluation harness (`data/eval_queries/` + `scripts/run_eval.py` — 70/70 passing; drove real fixes: section-aware chunking, orphan-embedding deletion, test-store hermeticity)
11. ⬜ Optional: plug an OpenAI-compatible LLM behind `app/chat/composer.py` (prompt templates already in `app/prompts/templates.py`)
12. ⬜ Optional: admin upload page (API works; UI would extend `demo-login.html`)
13. ⬜ Deployment prep + demo assets (screenshots, demo script)
